...
6. **CI/CD pipeline** — a real GitHub Actions (or equivalent) example with **one job per remote**, each gated on its own changed paths, so deploys are genuinely independent — not a single monolithic deploy job.
