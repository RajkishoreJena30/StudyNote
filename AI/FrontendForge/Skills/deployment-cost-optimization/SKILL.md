...
- **Independent deploys need independent CI jobs.** Gate each remote's deploy job on only its own changed paths (plus shared packages it depends on) so a fix in one remote doesn't force a redeploy — or even a test run — of every other remote.
